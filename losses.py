...
low_light_image = tf.random.normal(shape=(2, 128, 128, 3))
reference_image = tf.random.normal(shape=(2, 128, 128, 3))

output = enhancement_model([low_light_image, reference_image])


def euclidean_distance(x,y):
    distance = K.sum(tf.square(x - y), axis=-1, keepdims=True)
...
def reconstruction_loss(reconstructed_image, reference_image):
    ...
    Lr = K.abs(K.sum(reconstructed_image - reference_image, axis=(1, 2, 3)) / (128 * 128 * 3))
    Lr = K.mean(Lr)
    return Lr

def content_feature_loss(low_light_content, prediction_content):
    l2_loss = K.sum(K.square(prediction_content - low_light_content))
    return l2_loss
